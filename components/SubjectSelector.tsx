...
const SubjectSelector: React.FC<SubjectSelectorProps> = ({ subjects, onSelectSubject, onBack, level }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-900 text-gray-200">
      <div className="text-center w-full max-w-4xl mx-auto">
        <h1 className="text-4xl sm:text-5xl font-bold text-cyan-400 mb-2">المستوى: {level}</h1>
        <p className="text-xl text-gray-300 mb-12">الرجاء اختيار المادة الدراسية</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.map(({ name, count }) => (
              ...
              >
                ...
                <span className="text-sm font-semibold px-3 py-1 bg-gray-600 rounded-full">{count} عنصر</span>
              </button>
            ))}
          </div>
        ) : (
          <p className="text-lg text-yellow-400">لا توجد مواد دراسية متاحة لهذا المستوى.</p>
        )}
        ...
        <div className="mt-12">
          <button
            ...
            aria-label="الرجوع لاختيار المستوى"
          >
